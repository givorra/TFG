...
void sierpinskiPyramidSinEscalar(int iterations, POINT_CLOUD::Ptr sierpinski_set);
...
void sierpinskiTetrahedronCloudSinEscalar(int iterations, POINT_CLOUD::Ptr sierpinski_set);

int fractal_size = 100;
...
int main(int argc, char *argv[])
{
	if(argc == 3)
	{
		...
		else if(metodo == "tetra_pc_se")
		{
			POINT_CLOUD::Ptr sierpinski_cloud(new POINT_CLOUD());	
			sierpinskiTetrahedronCloudSinEscalar(iterations, sierpinski_cloud);
		}
		...
		else if(metodo == "pyra_pc_se")
		{
			POINT_CLOUD::Ptr sierpinski_cloud(new POINT_CLOUD());	
			sierpinskiPyramidSinEscalar(iterations, sierpinski_cloud);
		}
		else
			infoParms();
	}
...
void sierpinskiTetrahedronCloud(int iterations, POINT_CLOUD::Ptr sierpinski_set)
{
	...
	for(int i = 1; i <= iterations; i++)
	{
		...
		POINT_CLOUD::Ptr sierpinski_set_t(new POINT_CLOUD(*sierpinski_set));
  		kdtree.setInputCloud (sierpinski_set_t);
		...
		for(int j = 0; j < sierpinski_set->points.size(); j++)
		{
			...
			*/
			...
			if ( kdtree.nearestKSearch (p1_aux, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 && pointNKNSquaredDistance[0] == 0)
  			{
					repetido_p1_aux = true;
  			}
			if ( kdtree.nearestKSearch (p2_aux, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 && pointNKNSquaredDistance[0] == 0)
  			{
					repetido_p2_aux = true;
  			}
			if ( kdtree.nearestKSearch (p3_aux, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 && pointNKNSquaredDistance[0] == 0)
  			{
...
void sierpinskiPyramid(int iterations, POINT_CLOUD::Ptr sierpinski_set)
{
	...
	for(int i = 1; i <= iterations; i++)
	{
		cout << "fractal_size = " << fractal_size << "\n";
#if debug == 1 || debug == 2
		cout << "# Iteracion " << i << "...\n";
#endif
...
void sierpinskiPyramidSinEscalar(int iterations, POINT_CLOUD::Ptr sierpinski_set)
{
	...
	for(int i = 1; i <= iterations; i++)
	{
		...
		if(ply_writer.write("../models/sierpinski_pyramid_pc_sin_escalar/sierpinski_pyramid_" + numberToString(i) + ".ply", *sierpinski_set, false, false) < 0)
      		cerr << "Error writing .ply file" << std::endl;
